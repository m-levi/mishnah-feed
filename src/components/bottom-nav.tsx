...
export function BottomNav({ activeTab, onNavigate }: BottomNavProps) {
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-50 bg-[var(--card-bg)]/98 backdrop-blur-xl border-t border-[var(--border)] sm:hidden no-print">
        ...
        {items.map(({ key, icon: Icon, label }) => {
          ...
          return (
            <button
              ...
              className={`relative flex flex-col items-center justify-center min-w-[64px] h-full transition-colors duration-200 cursor-pointer active:scale-90 ${
                active ? "text-[var(--accent)]" : "text-[var(--muted)]"
              }`}
            >
              {active && (
                <span className="absolute top-0 left-1/2 -translate-x-1/2 w-8 h-[3px] rounded-full bg-[var(--accent)] tab-indicator" />
              )}
              <Icon
